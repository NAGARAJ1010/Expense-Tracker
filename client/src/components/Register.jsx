...
const Register = ({ isMatch }) => {
  ...
  const { fullName, email, password, confirmPassword } = useSelector(
  ...
  return (
    <>
      <span>
        <div
          className={`form-input-container ${
            !fullName && isMatch.fullName ? "error" : ""
          }`}
        >
        ...
        {!fullName && isMatch.fullName && (
          <p className="error-msg">Full Name required!</p>
        )}
      </span>
      <span>
        <div
          className={`form-input-container ${
            !email && isMatch.email ? "error" : ""
          }`}
        >
      ...
      <span>
        <div
          className={`form-input-container register ${
            !password && isMatch.password ? "error" : ""
          }`}
        >
          ...
          >
            ...
            >
              <span className="eye-icon block">
                <FontAwesomeIcon
                  icon={togglePasswordIcon ? faEye : faEyeSlash}
                />
              </span>
            </div>
          </div>
      ...
      <span>
        <div
          className={`form-input-container register ${
            (!confirmPassword && isMatch.confirmPassword[0]) ||
            isMatch.confirmPassword[1]
              ? "error"
              : ""
          }`}
        >
        ...
        {!confirmPassword && isMatch.confirmPassword[0] && (
          <p className="error-msg">Confirm Password required.</p>
        )}
        {isMatch.confirmPassword[1] && (
          <p className="error-msg">Password do not match.</p>
        )}
      </span>
    </>
  );
